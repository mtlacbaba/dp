...
Požadavky na nový editor budou vycházet z existujících řešení a potřeb, které na tuto aplikaci jsou kladeny v zadání práce. Z existujících řešení bych rád převzal několik osvědčených prvků a naopak některým prvkům bych se rád vyhnul, tak aby výsledný produkt produkt byl jednoduchý a intuitivní na ovládání a zároveň umožnil plně využít možnosti, které poskytuje workflow systém Kaleo, který je standardně obsažen v Liferay Portal CE. 

Požadavky kladené na nový editor můžeme rozdělit do dvou kategorií. Požadavky, které musí výsledný produkt splnit a na ty, které by mohl splňovat. Následující výčet obsahuje požadavky z první kategorie.

\begin{itemize}
\item Základním požadavkem, že nový editor bude plně kompatibilní s workflow systémem Kaleo. To znamená, že do editoru půjdou načítat již vložené workflow, půjde je dále upravovat, opět je zpátky ukládat do databáze portálu. Navíc by ještě mělo být možné exportovat upravované workflow v podobě definice ve formátu xml.
\item Dalším podstatným požadavkem kladeným na nový editor je, že bude pracovat v základní notaci BPMN. Tento požadavek je omezen samotným workflow systémem Kaleo, jenž se pořád vyvíjí a poskytuje zatím podporu pouze pro některé prvky.
\item Aplikace by měla po grafické stránce zapadat do portálu a proto by měla využít standardní prvky. Také by měla být umístěna v ovládacím panelu portálu v sekci konfigurace spolu s již existující správou workflow.
\item Aplikace by se měla jednoduše a intuitivně používat. Aplikaci by měl zvládnout používat každý uživatel, který zná notaci BPMN a orientuje v problematice workflow.

Další funkcionalita - zpět/vpřed,možnost používání funkcí při tvorbě skriptů a vlastních vložených skriptů, možnost také tuto funkcionalitu využívat pro šablony oznámení.

\end{itemize}

V následující seznamu jsou obsaženy požadavky z druhé kategorie, tedy ty, které nejsou nezbytně nutné, ale bylo užitečné, kdyby byly dostupné.

\begin{itemize}
\item Editor by mohl obsahovat podporu funkci zpět/vpřed.
\item Editor by měl umožňovat nastavit velikost plátna pro tvorbu workflow tak, aby bylo možné namodelovat i rozsáhlejší a komplikovanější procesy.
\item Editor pro tvorbu skriptů a šablon by mohl podporovat zvýrazňovaní syntaxe pro zvolené jazyky. Dále by také mohl obsahovat předdefinované funkce, které by bylo možné využít při tvorbě skriptů. Uživatel by také měl mít možnost tyto funkce rozšiřovat o své vlastní.
\item Měla by existovat možnost zkontrolovat správnost tvořeného workflow podle definice a případně uživatele upozornit na chyby bránící úspěšnému vložení workflow do systému.
\item Z pohledu použitelnosti aplikace by se mělo omezit používaní vyskakovacích dialogových oken, které mohou v nepřiměřené míře mohou znesnadňovat samotné používání aplikace, jak bylo ukázáno dříve. Viz. \autoref{fig:kaleo_designer_popup}
\end{itemize}
...
\chapter{Realizace}
Aplikace není tvořena klasickou třívrstvou architekturou, protože není potřeba řešit perzistenci dat. Výsledná workflow definice se buď vkládá přímo do portálu skrze portálové api nebo je možné výsledný XML soubor exportovat. Proto se v následující části zaměřím hlavně na popis prezentační vrstvy a také krátce vysvětlím, jak se používá portálové api.
...
Pro tento framework jsem rozhodl ze dvou důvodů. Prvním je, že standardní součástí Liferay Portal je na něm postaveno grafické rozhraní celého portálu a proto při jeho využití bude jednoduší zachovat stejný vzhled s portálem samotným. Druhým důvodem bylo, že Alloy UI poskytuje mnoho připravených komponent, které je možno využít. Z pohledu této práce je nejdůležitější komponentou Diagram Builder, viz. \autoref{fig:diagram_builder}. Pro potřeby aplikace musela být přizpůsobena.
\begin{figure}[htp]
\centering
